...
| **R2** | `shipped` | **Playlists — paste a YouTube _playlist_ URL and walk away — shipped 2026-08-21.** Expands the playlist, runs each track through the R1.5 pipeline, mirrors it into Jellyfin as a playlist named after the YouTube one; safe to re-run, backfills late-resolved tracks, one canonical file on disk. Carries the T-037 tag fix (ADR-028). **T-311 verified the whole acceptance checklist live against the real Jellyfin** — and surfaced+fixed **T-316** (the WSL/Windows resolve path-format gap that had kept any track from ever joining its playlist). Two residuals owner-accepted (item 7 failed-doesn't-stop → ADR-003 + tests; item 8 card → T-310 design gate). All tickets on `main`, 727 tests green. **Playlists only — migrate/clean is R2.5.** |
...
## Current release: none `in-build` — R2 (Playlists) shipped 2026-08-21; **R2.5 (Migrate/clean) is next** (`backlog` → move to `specing` to start)
...
(amended 2026-08-10)**, ADR-022. **R2 shipped 2026-08-21** (was `in-build` from 2026-08-14) — see below.
...
off that path, so it waits. **R2 shipped 2026-08-21** (all tickets on `main`, T-311 acceptance sweep
verified live); **R2.5 (Migrate/clean) is next** — move it `backlog` → `specing` to start.
...
### R2 (shipped) — closeout
- [x] All tickets T-300–T-312, T-315 built + integrated on `main` (prior sessions); **T-311 acceptance
      sweep done 2026-08-21** — the whole checklist driven live against the real Jellyfin, real music
      library untouched (temp `/mnt/c` library registered for the run, removed after)
- [x] **T-316** (found *by* T-311): the WSL(`/mnt/c`, `/`) vs Windows(`C:\`, `\`) resolve path-format gap
      that kept every landed track NOT_INDEXED forever — fixed (library-relative, case-folded tail match),
      re-verified live (playlist populated 0→5→6)
- [x] Two residuals owner-accepted: item 7 *failed-doesn't-stop* (a deleted video filters out at expansion —
      can't stage via a playlist; **ADR-003** + unit tests); item 8 *aggregate card* (T-310 design gate +
      batch-state data contract watched live)
- [x] **727 tests green on `main`**; Shipped 2026-08-21
