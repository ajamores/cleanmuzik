...
## Current release: none active — R2 (Playlists) is next
...
(amended 2026-08-10)**, ADR-022. Per the vocabulary rule, **no release is `in-build` right now.**
...
off that path, so it waits. R2's spec (`docs/r2/spec.md`) is signed off and `ready-for-agent` — **start
it by flipping its status to `in-build`.**
