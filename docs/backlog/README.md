...
- **Filed tickets** — findings that earned a ticket number, one file each. Triaging one into a
  ...
  - [`T-220.md`](T-220.md) — **EPIC: Shazam is the tag+art source of record; retire beets → mutagen writes** *(**IN BUILD, ADR-033 ratified 2026-08-26**. Subtraction, not a muziktest port — cleanmuzik already identifies with Shazam + 2-of-3 + fetches its art, then discards it. Wave: [`T-221`](T-221.md) widen Shazam record ✅DONE → [`T-222`](T-222.md) tag+art in `_accept` (AcoustID-only = 1 thin get_recording) ✅DONE (Option 1: Shazam every track, ADR-032/033 amended) → [`T-223`](T-223.md) mutagen ID3 writer → [`T-224`](T-224.md) retire beets ⚠carry-forward dedup+path-sanitization. Codec = T-225, deferred)*
