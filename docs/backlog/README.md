...
- **Filed tickets** — findings that earned a ticket number, one file each. Triaging one into a
  ...
  - [`T-035.md`](T-035.md) — Shazam as a backup fingerprint tier when AcoustID misses *(measured 4/5; ADR-019; **framing SUPERSEDED by the T-220 epic 2026-08-26** — Shazam is now primary, not a fallback; the 4/5 rescue + Frank Ocean evidence carried into ADR-033)*
  - [`T-220.md`](T-220.md) — **EPIC: Shazam is the tag+art source of record; retire beets → mutagen writes** *(**DONE 2026-08-27** — all four waves shipped. Subtraction, not a muziktest port — cleanmuzik already identifies with Shazam + 2-of-3 + fetches its art, then discards it. Wave: [`T-221`](T-221.md) widen Shazam record ✅DONE → [`T-222`](T-222.md) tag+art in `_accept` (AcoustID-only = 1 thin get_recording) ✅DONE (Option 1: Shazam every track, ADR-032/033 amended) → [`T-223`](T-223.md) mutagen ID3 writer ✅DONE (import `write` off; ID3/APIC via mutagen; thumbnail centre-cropped) → [`T-224`](T-224.md) ✅DONE **re-scoped 2026-08-27**: retired the 4 dead tag plugins (lastgenre/fetchart/embedart/lyrics), beets KEPT as the import framework; lyrics now wired from Shazam; Pillow pinned. **Full beets removal split out to [`T-226`](T-226.md)** (spine rewrite — its own sub-wave, ADR-033 decision 3 amended). Codec = T-225, deferred)*
  - [`T-226.md`](T-226.md) — **Remove beets entirely (replace the import framework)** *(NOT SCHEDULED — the "throw the knife away" job split out of T-224 2026-08-27. beets is the import framework (mover + memory/dedup + suggester + `ImportSession` driver), not a tagger; a bounded multi-day spine rewrite whose payoff is dead-weight removal, not a user-facing win. Holds the 3-agent research: mover=cheap, memory=needs a recording-id frame + library scan, suggester=medium (MBIDs already free from `_lookup_dominance`). Owner decision to schedule)*
  ...
  - [`T-048.md`](T-048.md) — durable per-track rows for the batch card: landed/gone row *detail* survives a restart, not just the tally *(untriaged; from the T-310 build; would revisit **ADR-027 seam 5**'s aggregate-only snapshot + the **ADR-015** landing-receipt tension — needs a durable per-track read, not a column add)*
  - [`T-210.md`](T-210.md) — MusicBrainz retry-ladder bound **(speed half — DONE 2026-08-25, ADR-031)** + shared rate limiter / ISRC **(correctness half — OPEN)** *(the 18–34s tail was beets' `Retry(total=6)` backoff, not an uncapped socket — the 10s timeout already existed. `mb_retry.py` bounds it to one retry (6→1); tail collapses to ~one timeout, degrades to blank-year/park. Caps the tail, does not lower the median — that's T-219/T-035)*
  - [`T-208.md`](T-208.md) — collapse the MB candidate **fan-out** (winner-only hydration; the `mb_search.py` finding-#4 fix, applied to acquire) *(**DONE — integrated to `main` 2026-08-24**: 33→6 MB `get_recording`, 0 decision/tag drift, 743 tests green on `main`; ADR-030. Step 5 fpcalc fold-in deferred → T-210 orbit)*
  ...
  - [`T-218.md`](T-218.md) — **measure-first gate:** profile the identify block for the real per-sub-step split (AcoustID / Shazam / MB / LLM) before pulling any speed lever *(MEASURED 2026-08-23 → MB named the target → T-208. **Now also holds the 2026-08-24 live Lever-A-vs-B comparison:** muziktest ~8× faster wall-clock, same 10/4 split, complementary AcoustID/Shazam coverage — the gap is closable in cleanmuzik via T-208+T-210+T-219+T-035, no rewrite)*
  - [`T-219.md`](T-219.md) — **corroboration fast-path:** skip the LLM reconcile when the fingerprint identity already agrees with the source title *(**DONE — integrated to `main` 2026-08-25**, **ADR-032**, 756 green — `_corroboration_fast_path` in `import_seam.py`. Acceptance compare passed: ~20% faster on fast-pathed tracks (5.4s/track), 9/16 fast-pathed, outcome-neutral (the land/park flips are MB-503 noise, matched by a same-code control). Transplanted from muziktest's Confidence gate)*
