...
- **Filed tickets** — findings that earned a ticket number, one file each. Triaging one into a
  ...
  - [`T-219.md`](T-219.md) — **corroboration fast-path:** skip the LLM reconcile when the fingerprint identity already agrees with the source title *(**BUILT 2026-08-25** on `perf/t-219-corroboration-fast-path`, **ADR-032**, 755 green — `_corroboration_fast_path` in `import_seam.py`; awaiting the ADR-030 corpus compare + integration. Transplanted from muziktest's Confidence gate, fast-pathed 8/12 there; secondary to T-210, engine-touching, same acceptance bar as T-208)*
