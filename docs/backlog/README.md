...
- **Filed tickets** — findings that earned a ticket number, one file each. Triaging one into a
  ...
  - [`T-210.md`](T-210.md) — MusicBrainz rate limiting **+ per-call timeout** *(**PRIMARY remaining speed lever, 2026-08-24**: T-208 killed the MB call-count; the per-call latency tail — spikes to 18–34s — is now the #1 time cost. The timeout caps it. Do this first)*
  - [`T-208.md`](T-208.md) — collapse the MB candidate **fan-out** (winner-only hydration; the `mb_search.py` finding-#4 fix, applied to acquire) *(**BUILT + acceptance-compare PASSED 2026-08-24** on branch `t208-mb-fanout-collapse`: 33→6 MB `get_recording`, 0 decision/tag drift, 743 tests green; awaiting integration to `main`. Step 5 fpcalc fold-in deferred)*
