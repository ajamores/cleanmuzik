...
- **Resolved 2026-08-12 (T-207 session).** The key was first placed under the SDK-default
  `ANTHROPIC_API_KEY` and the code half of this ticket had never been built, so reconcile was silently
  degraded. Fixed: owner renamed the `.env` var to `ANTHROPIC_APIKEY`; added the `anthropic_apikey` field
  to `config.py` (mirrors `lastfm_apikey`, binds env `ANTHROPIC_APIKEY`); added it to the `config loaded:`
  boot log (`main.py`); documented it in `.env.example`. Verified readable in-process (`set`), server suite
  524 green with reconcile held off. **Owner must restart uvicorn** (`--reload` won't reload `.env`) and
  confirm the boot log now reads `anthropic_apikey=set` before running T-209.
...
### T-208 / T-214 / T-215 — the speed follow-ons (filed to `docs/backlog/`, R1.5 shipped)
T-209 surfaced the identity-speed finding; a design council (2026-08-13) split it into three by risk, and
the owner deferred the engine change. They live in the backlog now (this is a shipped release's ledger, not
a parking lot):
- [`backlog/T-215.md`](../backlog/T-215.md) — **Shazam hoist** (safe, non-engine, ~2s + kills the hang tail). Buildable anytime.
- [`backlog/T-214.md`](../backlog/T-214.md) — **narrate the identify freeze** (safe, perceived speed, biggest felt win). Buildable anytime.
- [`backlog/T-208.md`](../backlog/T-208.md) — **candidate de-hydration** (the ~4–5s lever; the **only** piece that touches the tagging engine → deferred and conditional, clears its own §7 gate, not bolted to R2).
