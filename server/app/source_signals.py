...
class SourceSignals:
    ...
    def from_info(cls, info: dict) -> "SourceSignals":
        ...
        in order, to a title parse then the Topic uploader (`yt_artist`) / the raw
        title (`yt_title`). Every field is set explicitly, so the result never carries
        a missing key regardless of how sparse `info` was.
        """
        ...
        # yt_artist: structured field → title split → Topic uploader → None.
