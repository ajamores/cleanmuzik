...
# `shazam.py` lives at server/app/shazam.py, so server/ is one dir up — the same
# __file__ anchoring config.py uses, independent of the launch cwd.
...
# imported (the 3.12 venv has no `app` package).
SHAZAM_PYTHON = SERVER_DIR / ".venv-shazam" / "bin" / "python3.12"
RUNNER_PATH = Path(__file__).resolve().parent / "shazam_runner.py"

# Default hard timeout. Tunable per-call (T-204 binds the injected `shazam_fn`)
# and operationally via `SHAZAM_TIMEOUT_S` in the environment.
DEFAULT_TIMEOUT_S = float(os.environ.get("SHAZAM_TIMEOUT_S", "8.0"))
