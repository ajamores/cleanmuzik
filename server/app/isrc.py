...
2. **1 request/second — a shared floor.** ADR-001 caps identification traffic at 1/sec.
   This is a *direct* MusicBrainz call outside beets' own MB limiter, so it keeps its OWN
   monotonic-clock throttle (a module-level gate) to make sure its calls never exceed the
   floor. The pipeline is serial (ADR-001/ADR-022), so one gate is sufficient; the lock
   only guards the shared timestamp.
...
# Matches `app.artwork._UA` verbatim — MusicBrainz etiquette (and ADR-001) requires a
# descriptive User-Agent, not python-requests' default. Kept as a local copy to avoid
# importing `artwork` (which pulls `beetsplug` in at module scope).
_UA = "CleanMuzik/0.1 (personal music library; +https://github.com/)"
