...
class EventBus:
    ...
    def _dispatch(self, loop, subscribers, item) -> None:
        ...
        for queue in subscribers:
            if loop is None:
                # No loop bound means no live subscribers can exist — this is
                # defensive. The buffer already holds the event for replay.
                continue
            loop.call_soon_threadsafe(queue.put_nowait, item)

    # --- loop side ---------------------------------------------------------

    async def stream(self, job_id: str, *, ping_interval: float = PING_INTERVAL):
