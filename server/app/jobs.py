...
def run_resolve(
    ...
    **failed** resolve deliberately keeps it and returns the review to `pending` — the
    ...
    if review is None:
        # Claimed a moment ago by the route, so this is a torn/vanished row rather
        # than a normal miss — nothing to resume and no job to attribute it to.
        logger.error("resolve for review %s found no row", review_id)
        return JobState(review_id, STATUS_ERROR, error=f"no review {review_id}")
    ...
    try:
        ...
        # Snapshot the library BEFORE the import: this is what tells the new copy from
        # the old ones afterwards. For `replace` it is load-bearing — after landing,
        # a query by recording id returns BOTH copies, and deleting "the duplicate"
        # without this set could delete the file we just landed.
        before = items_for_recording(lib, request.recording_id)
        before_ids = {item.id for item in before}

        if request.choice == CHOICE_REPLACE and len(before) > 1:
            # Spec §6 and ADR-009's addendum both say `replace` deletes "THE existing
            # library file" — singular. They don't say which one to delete when two
            # library files share a recording id, and that state is reachable: it is
            # exactly what `keep_both` creates. Deleting all of them would destroy the
            # copy the owner deliberately kept as distinct — an ADR-009-class loss
            # arriving through a door the ADR didn't anticipate. So refuse, before the
            # import lands anything: a click that cannot identify its target is not
            # consent to delete every candidate for it. Checked here rather than in
            # _replace_existing so nothing has landed and nothing needs unwinding.
...
class JobWorker:
    ...
    def submit_resolve(self, job_id: str, review_id: str, request: ResolveRequest) -> None:
        ...
        **Everything before the `put` happens synchronously inside the resolve request,
        ...
        self._queue.put(_ResolveWork(review_id, request))
        ...
        while True:
            ...
            try:
                ...
                if isinstance(item, _PipelineWork):
                    ...
                    run_resolve(
                        item.review_id, item.request, store=self._store,
                        registry=self.registry, settings=self._settings, bus=self.bus,
                    )
                else:
