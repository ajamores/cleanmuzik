...
from app.events import EventBus, candidate_row, format_sse
...
# --- terminal hint: an evicted completed job must not hang -------------------


def test_evicted_completed_job_with_terminal_hint_closes_immediately():
    # A completed job's channel is eventually dropped by the cap (and all channels are
    # gone after a restart). Reconnecting then, with the durable status already terminal,
    # must replay nothing and return — NOT fabricate a fresh open channel that blocks
    # forever on a close() that already fired on the discarded one (the bug: the stream
    # would emit only pings indefinitely and the card would never see a terminal event).
    bus = EventBus(cap=2)
    bus.publish("old", "job.queued", {"job_id": "old", "url": "u"})
    bus.close("old")
    bus.publish("a", "job.queued", {"job_id": "a", "url": "u"})
    bus.publish("b", "job.queued", {"job_id": "b", "url": "u"})  # evicts "old"
    assert "old" not in bus._channels  # oldest, no live subscriber → gone

    async def run():
        frames = [frame async for frame in bus.stream("old", terminal=True)]
        return frames

    # wait_for so a regression fails loudly (TimeoutError) instead of hanging the suite.
    frames = asyncio.run(asyncio.wait_for(run(), timeout=2.0))
    assert frames == []


def test_terminal_hint_still_replays_a_present_channel():
    # The hint only short-circuits when the channel is GONE. A just-finished job whose
    # channel is still resident must replay its full buffer, then close.
    bus = EventBus()
    bus.publish("j", "job.queued", {"job_id": "j", "url": "u"})
    bus.publish("j", "track.done", {"job_id": "j", "path": "/x.mp3", "tags": {}})
    bus.close("j")

    events = _drain(bus, "j", terminal=True)
    assert [name for name, _ in events] == ["job.queued", "track.done"]
...
def test_candidate_row_is_the_canonical_four_key_shape():
    # Both builders (park-time + id-only fallback) and T-014's re-hydration route
    # through candidate_row, so the spec §6 key set lives here alone. Lock it —
    # album / year / art_url were removed by ADR-010 (a recording is not a release);
    # this test is what stops a future change silently re-adding the structural nulls.
